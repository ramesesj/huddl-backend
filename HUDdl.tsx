...
// ─── Semantic search tip chips ───────────────────────────────────────────────
const SEARCH_TIPS = [
  { label: "2BR",         query: "2 bedroom" },
  { label: "Studio",      query: "studio" },
  { label: "Section 8",   query: "section 8" },
  { label: "Affordable",  query: "affordable" },
  { label: "Pet Friendly",query: "pet friendly" },
  { label: "Oakland",     query: "oakland" },
  { label: "Hayward",     query: "hayward" },
  { label: "Berkeley",    query: "berkeley" },
  { label: "Fremont",     query: "fremont" },
  { label: "Senior",      query: "senior" },
  { label: "LIHTC",       query: "lihtc" },
  { label: "Available Now",query:"available now" },
  { label: "Veteran",     query: "veteran" },
  { label: "Waitlist",    query: "waiting list" },
  { label: "1BR",         query: "1 bedroom" },
  { label: "3BR",         query: "3 bedroom" },
  { label: "HUD",         query: "hud" },
  { label: "Parking",     query: "parking" },
  { label: "Laundry",     query: "laundry" },
  { label: "Disabled/ADA",query: "ada" },
  { label: "Furnished",   query: "furnished" },
  { label: "Short Term",  query: "short term" },
  { label: "Dublin",      query: "dublin" },
  { label: "Livermore",   query: "livermore" },
  { label: "USDA Rural",  query: "usda" },
];

// Semantic aliases shown in the info panel
const ALIAS_EXAMPLES: { term: string; aliases: string[] }[] = [
  { term: "Bedroom",    aliases: ["bed","bd","br","bdrm","bedroom","1br","2br","3br"] },
  { term: "Bathroom",   aliases: ["bath","ba","full bath","half bath","powder room"] },
  { term: "Affordable", aliases: ["low income","subsidized","below market","ami","bmi"] },
  { term: "Section 8",  aliases: ["s8","hcv","voucher","housing voucher","hap"] },
  { term: "Available",  aliases: ["vacant","for rent","leasing now","move-in ready","immediate"] },
  { term: "Pet OK",     aliases: ["pet","pets","dogs allowed","cats allowed","pets welcome"] },
  { term: "Studio",     aliases: ["efficiency","eff","bachelor","0br","open plan"] },
  { term: "Apartment",  aliases: ["apt","flat","unit","suite","rental"] },
  { term: "Senior",     aliases: ["elderly","55+","62+","retirement","age restricted"] },
  { term: "Parking",    aliases: ["garage","carport","off-street","covered parking"] },
];

function SemanticTips({ onSelect }: { onSelect: (q: string) => void }) {
  const [showAliases, setShowAliases] = useState(false);
  return (
    <div className="max-w-6xl mx-auto px-4 pb-3 pt-1">
      {/* Quick-tap chips */}
      <div className="flex flex-wrap gap-1.5 mb-2">
        {SEARCH_TIPS.map(({ label, query }) => (
          <button
            key={label}
            onClick={() => onSelect(query)}
            className="text-[11px] font-medium px-2.5 py-1 rounded-full bg-slate-100 hover:bg-blue-100 hover:text-blue-700 text-slate-600 transition-colors"
          >
            {label}
          </button>
        ))}
      ...
      <button
        ...
        className="text-[11px] text-slate-400 hover:text-blue-500 transition-colors underline-offset-2 underline"
      >
        {showAliases ? "▲ Hide search synonyms" : "▼ What words can I search?"}
      </button>
      {showAliases && (
        <div className="mt-2 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2">
          {ALIAS_EXAMPLES.map(({ term, aliases }) => (
            <div key={term} className="bg-white border border-slate-100 rounded-xl p-2 shadow-sm">
              <p className="text-[11px] font-semibold text-blue-700 mb-1">{term}</p>
    ...
    <div className="flex flex-col items-center justify-center py-28 gap-4">
      ...
      <div className="text-center">
        ...
        <p className="text-xs text-slate-300 mt-1">
          Deep-crawling 25+ sites (every page) + HUD databases · takes 1–2 minutes
        </p>
      </div>

      {/* Progress bar */}
      <div className="w-64 h-1.5 bg-slate-100 rounded-full overflow-hidden">
        <div className="h-full bg-blue-400 rounded-full animate-[progress_90s_linear_forwards]"
...
export default function HUDdl() {
  ...
  const fetchListings = useCallback(async (q: string, source: string) => {
    ...
    try {
      // The backend crawls in the background and answers 202 until the
      // first crawl finishes (~1–2 min). Poll instead of giving up.
      // Network errors early on are usually Render cold starts — retry those too.
      const MAX_ATTEMPTS = 40; // × 5s ≈ 3.5 min ceiling
      for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
        ...
        try {
          ...
          if (attempt < 5) { await sleep(5000); continue; } // cold start grace
          throw new Error("network");
        }
        if (res.status === 202) { await sleep(5000); continue; } // still crawling
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/20 to-indigo-50 font-sans">
        ...
        <div className="max-w-6xl mx-auto px-4 py-3 flex flex-col sm:flex-row items-center gap-3">
          ...
          <div className="flex-1 relative max-w-xl w-full">
            ...
            <input
              ...
              placeholder="Try: 2br Oakland, studio affordable, s8 hayward, pet friendly 1 bed…"
      ...
      {/* Semantic search tips — always visible below header */}
      <div className="bg-white/70 backdrop-blur border-b border-slate-100">
        <SemanticTips onSelect={(q) => { setQuery(q); if (crawled) fetchListings(q, tab === "all" ? "all" : tab); }} />
      </div>
      ...
      <main className="max-w-6xl mx-auto px-4 py-6">
        ...
        {!crawled && !loading && (
          <div className="flex flex-col items-center justify-center py-28 text-center">
            ...
            <p className="text-xs text-slate-300 mt-4">Press <strong>Start Search</strong> to begin · takes 1–2 minutes</p>
          </div>
        )}
