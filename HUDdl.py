...
# ── Listing-specific links (v6.4.2) ──────────────────────────────────────────
# Cards should link to THE listing (its floor plan, detail, or apply page),
# not the site's general page. This finds the most listing-specific link
# inside a row/card/paragraph; extractors fall back to the page URL only
# when an element has no usable link of its own.
_LISTING_LINK_RE = re.compile(
    r"(floor|unit|plan|avail|apply|lease|listing|detail|view|rent|tour)", re.I)

def _best_link(el, base_url: str) -> str:
    ...
    # First pass: links that look listing-specific by text or path.
    for a in links:
        href = resolve(a)
        if href and (_LISTING_LINK_RE.search(a.get_text(" ", strip=True) or "")
                     or _LISTING_LINK_RE.search(urllib.parse.urlparse(href).path)):
            return href
    # Second pass: any real link inside the element.
    for a in links:
        href = resolve(a)
        if href:
            return href
...
def _extract_floor_plan_rows(url: str, soup, text: str) -> list[Unit]:
    ...
    for table in soup.find_all("table"):
        ...
        for row in rows[1:]:
            ...
            u.url          = _best_link(row, url) or u.url
            units.append(u)
    if units:
        return _dedup(units)
    ...
    for sel in card_selectors:
        ...
        for card in cards[:100]:
            ...
            u.url          = _best_link(card, url) or u.url
            units.append(u)
    if len(units) >= 2:
        return _dedup(units)
    ...
    for tag in soup.find_all(["p","li","dd","dt","span","div"]):
        ...
        u.unit_label   = _make_label(u)
        u.description  = ct[:200]
        u.url          = _best_link(tag, url) or (_best_link(tag.parent, url)
                                                  if tag.parent else "") or u.url
        units.append(u)
...
def _extract_directory_cards(url: str, soup, text: str) -> list[Unit]:
    ...
    for card in cards:
        ...
        href = link["href"]
        full = _best_link(card, url) or (
            href if href.startswith("http") else urllib.parse.urljoin(url, href))
        key  = (name.lower(), full)
        if key in seen:
            continue
        seen.add(key)
