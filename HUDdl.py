...
def _in_alameda(u: dict) -> bool:
    ...
    # No structured city/state/zip — try to parse the free-text address.
    ...
    if m:
        ...
        if st != "CA":
            return False
        return _zip_in_alameda(zc) or _city_in_alameda(c)
...
async def _fetch_rendered_many(urls: list[str]) -> dict[str, Optional[str]]:
    """Return {url: rendered_html_or_None} using one shared Chromium instance."""
    out: dict[str, Optional[str]] = {u: None for u in urls}
    if not urls or not _headless_available():
        return out
    try:
        async with async_playwright() as pw:
            ...
            for url in urls:
                ...
                try:
                    await page.goto(url, timeout=RENDER_NAV_MS,
                                    wait_until="domcontentloaded")
                    try:
                        await page.wait_for_load_state("networkidle",
                                                       timeout=RENDER_IDLE_MS)
                    except Exception:
                        pass  # busy sites never go idle — settle time below
                    await page.wait_for_timeout(RENDER_SETTLE_MS)
                    out[url] = await page.content()
                    print(f"  [render] {netloc}: rendered OK "
                          f"({len(out[url] or '')} bytes)")
                except Exception as e:
                    print(f"  [render] {netloc} → {type(e).__name__}: {e}")
            await browser.close()
    except Exception as e:
...
def _base_unit(url: str, soup, text: str) -> Unit:
    ...
    # v6.2: pull structured city/state/zip out of the address when possible,
    # so the Alameda County filter has something to check.
    m = CITY_ST_ZIP_RE.search(u.address)
    if m:
...
def _extract_directory_cards(url: str, soup, text: str) -> list[Unit]:
    ...
    for card in soup.find_all(["div", "article", "li", "section"],
        ...
        # skip containers that themselves contain matching cards (outer wrappers)
        if card.find(["div", "article", "li", "section"], class_=_DIR_CARD_CLASS):
            continue
        link    = card.find("a", href=True)
        heading = card.find(["h1", "h2", "h3", "h4", "h5", "strong"])
        name    = heading.get_text(" ", strip=True) if heading else ""
        if not link or not (3 <= len(name) <= 90):
            continue
        href = link["href"]
...
async def crawl_web() -> list[Unit]:
    # v6.3: JS-only sites go through headless Chromium; the rest use aiohttp.
    ...
    pages    = await _fetch_all(plain_urls)
    rendered = await _fetch_rendered_many(js_urls)
    ...
    for url in js_urls:
        ...
        if html:
            units = parse_page(url, html, rendered=True)
            # Rendering succeeded but extractors found nothing usable →
            # still give the user a way in.
            if len(units) == 1 and units[0].unit_label in ("Unit", "") \
               and not units[0].price and not units[0].bedrooms:
                units = [_link_card(url)]
            results.extend(units)
            print(f"  [web] {urllib.parse.urlparse(url).netloc}: "
                  f"{len(units)} unit(s) (headless)")
        else:
            results.append(_link_card(url))
            print(f"  [web] {urllib.parse.urlparse(url).netloc}: "
                  "render failed → link card")
    return results
